...
export default function ProductCard({ product, layout = 'grid' }: ProductCardProps) {
  ...
  if (layout === 'scroll') {
    return (
      <Link href={`/product/${product.id}`} passHref>
        <div className="group flex h-full flex-1 flex-col gap-4 rounded-xl bg-content-light shadow-sm border border-text-light/10 min-w-64 transition-shadow hover:shadow-lg cursor-pointer">
          ...
          <div className="flex flex-col flex-1 justify-between p-4 pt-0 gap-4">
            ...
            <button
              ...
              className="flex w-full min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-background-light border border-text-light/20 text-sm font-bold hover:bg-primary hover:text-white hover:border-primary transition-colors"
            >
  ...
  return (
    <Link href={`/product/${product.id}`} passHref>
      <div className="group flex flex-col gap-4 rounded-xl bg-content-light shadow-sm border border-text-light/10 transition-shadow hover:shadow-xl cursor-pointer h-full">
        <div className="relative w-full overflow-hidden rounded-t-xl">
          <WishlistButton product={product} renderedPage="productCard" />
