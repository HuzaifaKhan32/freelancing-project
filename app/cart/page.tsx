        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {cart.map((item) => (
                <div key={`${item.product.id}-${item.size}-${item.color}`} className="flex items-start gap-4 border-b pb-4">
                  <Link href={`/product/${item.product.id}`}>
                    <img src={item.product.images[0].src} alt={item.product.images[0].alt} className="w-24 h-32 object-cover rounded-lg" />
                  </Link>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-gray-100 dark:bg-gray-800 p-6 rounded-lg">
              ...
              <button className="w-full mt-6 bg-primary text-white py-3 rounded-lg font-bold hover:bg-primary/90">
                Checkout
              </button>
            </div>
          </div>
